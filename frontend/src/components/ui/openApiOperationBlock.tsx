...
const OpenApiOperationBlock = ({ 
  ...
  if (isEditing) {
    return (
      <Card className="my-6 border-2 border-primary shadow-xl animate-in fade-in zoom-in duration-200">
        <CardHeader className="flex justify-between items-center bg-[var(--grud-surface-alt)] dark:bg-[var(--grud-code-bg)] px-6 py-4">
  ...
  return (
    <Card className="my-6 border border-[var(--grud-border-color)]/50 dark:border-[var(--grud-code-border)]/50 bg-white/50 dark:bg-[var(--grud-code-bg)]/50 backdrop-blur-md shadow-lg overflow-hidden group hover:shadow-xl transition-all duration-300">
      <CardHeader className="p-0">
        <div className="flex items-center justify-between p-4 bg-[var(--grud-surface-alt)]/80 dark:bg-[var(--grud-code-bg)]/80 border-b border-[var(--grud-border-color)]/50 dark:border-[var(--grud-code-border)]/50">
          <div className="flex items-center gap-3">
            ...
            <code className="text-sm font-mono bg-[var(--grud-surface-alt)] dark:bg-[var(--grud-code-bg)] px-3 py-1 rounded-md text-[var(--grud-text-secondary)] dark:text-[var(--grud-text-secondary)]">
              {operation.endpoint}
            </code>
          </div>
          {canEdit && (
        ...
        <div className="px-6 py-4">
          <h3 className="text-xl font-bold text-[var(--grud-text)] dark:text-[var(--grud-text)]">
            {operation.summary || 'No summary'}
          </h3>
          {operation.description && (
            <p className="mt-2 text-[var(--grud-text-secondary)] dark:text-[var(--grud-text-secondary)] text-sm leading-relaxed">
              {operation.description}
            </p>
          )}
        </div>
            ...
            <div className="py-6 space-y-8">
              ...
              {operation.parameters && operation.parameters.length > 0 && (
                <div className="space-y-4">
                  <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--grud-text-secondary)]">Parameters</h4>
                  <div className="overflow-x-auto rounded-xl border border-[var(--grud-border-color)] dark:border-[var(--grud-code-border)]">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className="bg-[var(--grud-surface-alt)]/50 dark:bg-[var(--grud-code-bg)]/50">
                        <tr>
                          <th className="px-4 py-3 text-left text-xs font-bold text-[var(--grud-text-secondary)] uppercase">Name</th>
                          <th className="px-4 py-3 text-left text-xs font-bold text-[var(--grud-text-secondary)] uppercase">In</th>
                          <th className="px-4 py-3 text-left text-xs font-bold text-[var(--grud-text-secondary)] uppercase">Description</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-700 bg-white/50 dark:bg-black/20">
                        {operation.parameters.map((param, index) => (
                          <tr key={index} className="hover:bg-[var(--grud-surface-alt)]/50 dark:hover:bg-white/5">
                            <td className="px-4 py-4 whitespace-nowrap">
                              <div className="flex items-center gap-2">
                                <code className="text-sm font-bold text-primary">{param.name}</code>
                                {param.required && <span className="text-[var(--grud-error)] font-bold">*</span>}
                              </div>
                            </td>
                            <td className="px-4 py-4 whitespace-nowrap">
                              <Chip size="sm" variant="flat" color="secondary">{param.in}</Chip>
                            </td>
                            <td className="px-4 py-4 text-sm text-[var(--grud-text-secondary)] dark:text-[var(--grud-text-secondary)]">
                              {param.description || '-'}
                            </td>
                          </tr>
                        ))}
              ...
              {operation.requestBody && (
                <div className="space-y-4">
                  <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--grud-text-secondary)]">Request Body</h4>
                  {operation.requestBody.content && Object.entries(operation.requestBody.content).map(([contentType, content]) => (
                    <div key={contentType} className="space-y-3">
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-[var(--grud-text-secondary)] font-mono">Content-Type:</span>
              ...
              {operation.responses && (
                <div className="space-y-4">
                  <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--grud-text-secondary)]">Responses</h4>
                  <div className="grid grid-cols-1 gap-4">
                    {Object.entries(operation.responses).map(([status, response]) => (
                      <div key={status} className="p-4 rounded-xl border border-[var(--grud-border-color)] dark:border-[var(--grud-code-border)] bg-[var(--grud-surface-alt)]/30 dark:bg-[var(--grud-code-bg)]/30 space-y-3">
                        <div className="flex items-center gap-3">
                          <Chip 
                            size="sm" 
                        ...
                        {response.content && Object.entries(response.content).map(([contentType, content]) => (
                          <div key={contentType} className="mt-2 pl-4 border-l-2 border-[var(--grud-border-color)] dark:border-[var(--grud-code-border)]">
                            <div className="text-xs text-[var(--grud-text-secondary)] mb-2">{contentType}</div>
                            {content.schema && renderJsonSchema(content.schema)}
                          </div>
                        ))}
                      </div>
            ...
            <div className="py-6 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--grud-text-secondary)] flex items-center gap-2">
                    Request Parameters
                  </h4>
                  <div className="space-y-4">
                    {operation.parameters?.map(param => (
                      <Input
                        ...
                        classNames={{
                          inputWrapper: "border-[var(--grud-border-color)] dark:border-[var(--grud-code-border)] hover:border-primary transition-colors",
                          label: "text-[var(--grud-text-secondary)] font-medium"
                        }}
                      />
                    ))}
                    ...
                    {operation.requestBody && (
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-[var(--grud-text-secondary)]">Request Body (JSON)</label>
                        <EnhancedCodeEditor
                ...
                {/* Test Result */}
                <div className="space-y-6">
                  <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--grud-text-secondary)]">Response</h4>
                  {testResponse ? (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                      {testResponse.error ? (
                        <div className="p-4 rounded-xl bg-[color-mix(in_srgb,var(--grud-error)_10%,transparent)] dark:bg-red-900/20 border border-[color-mix(in_srgb,var(--grud-error)_30%,transparent)] dark:border-red-800">
                          <p className="text-[var(--grud-error)] dark:text-[var(--grud-error)] font-medium">{testResponse.error}</p>
                        </div>
                      ) : (
                        <>
                          <div className="flex items-center gap-3">
                          ...
                          <div className="rounded-xl overflow-hidden border border-[var(--grud-border-color)] dark:border-[var(--grud-code-border)]">
                            <EnhancedCodeEditor
                    ...
                    <div className="h-[400px] flex flex-col items-center justify-center border-2 border-dashed border-[var(--grud-border-color)] dark:border-[var(--grud-code-border)] rounded-3xl text-[var(--grud-text-secondary)]">
