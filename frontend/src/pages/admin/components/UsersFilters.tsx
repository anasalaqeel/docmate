  ...
  return (
    <div className="mb-6">
      ...
      <div className="flex flex-wrap gap-3">
        {/* Status Filter */}
        <div className="flex items-center gap-2 px-4 py-2 rounded-lg" style={{ background: 'var(--grud-surface-alt)' }}>
          <span className="text-sm font-medium" style={{ color: 'var(--grud-text-secondary)' }}>Status:</span>
          <Dropdown>
            <DropdownTrigger>
              <Button
                size="sm"
                style={{ 
                  background: filters.status === "active" ? 'rgba(var(--grud-success-rgb), 0.1)' : filters.status === "inactive" ? 'rgba(var(--grud-error-rgb), 0.1)' : 'var(--grud-surface)',
                  color: filters.status === "active" ? 'var(--grud-success)' : filters.status === "inactive" ? 'var(--grud-error)' : 'var(--grud-text-secondary)',
        ...
        {/* Roles Filter */}
        <div className="flex items-center gap-2 px-4 py-2 rounded-lg" style={{ background: 'var(--grud-surface-alt)' }}>
          <span className="text-sm font-medium" style={{ color: 'var(--grud-text-secondary)' }}>Roles:</span>
          <Dropdown>
            <DropdownTrigger>
              <Button
                size="sm"
                style={{ 
                  background: filters.roleIds && filters.roleIds.length > 0 ? 'rgba(var(--grud-primary-rgb), 0.1)' : 'var(--grud-surface)',
                  color: filters.roleIds && filters.roleIds.length > 0 ? 'var(--grud-primary)' : 'var(--grud-text-secondary)',
                  borderColor: filters.roleIds && filters.roleIds.length > 0 ? 'transparent' : 'var(--grud-border-color)'
                }}
                endContent={<FunnelIcon className="w-3 h-3 opacity-40" />}
              >
      ...
      {hasActiveFilters && (
        <div 
          ...
          style={{ background: 'rgba(var(--grud-primary-rgb), 0.05)', border: '1px solid rgba(var(--grud-primary-rgb), 0.1)' }}
        >
