...
if [ -f "$ENV_FILE" ]; then
  ...
  echo "Warning: $ENV_FILE not found locally. Ensure .env.docker exists on the server."
fi
...
echo "Access your app at http://${SERVER_IP}:${FRONTEND_PORT:-80}"
echo "API docs at http://${SERVER_IP}:${BACKEND_PORT:-8000}/v1/docs"
